...
function (formula, data, weights, start, ..., subset, na.action, 
    ...
    prior.mean = 0, prior.scale = 2.5, prior.df = 1, prior.mean.for.cutpoints = 0, 
    ...
    scaled = TRUE, n.iter = 100) 
{
    ...
    if (missing(start)) {
        ...
        fit <- switch(method, logistic = bayesglm.fit(X, y1, 
            ...
            probit = bayesglm.fit(X, y1, wt, family = binomial("probit"), 
                ...
                control = glm.control(maxit = n.iter)), cloglog = bayesglm.fit(X, 
                y1, wt, family = binomial("probit"), offset = offset, 
                intercept = TRUE, prior.mean = prior.mean, prior.scale = prior.scale, 
                prior.df = prior.df, prior.mean.for.intercept = 0, 
                prior.scale.for.intercept = 10, prior.df.for.intercept = 1, 
                scaled = scaled, control = glm.control(maxit = n.iter)), 
    ...
    res <- optim2(start, fmin, gmin, method = "BFGS", hessian = Hess, 
        ...)
    ...
    fit <- list(coefficients = beta, zeta = zeta, deviance = deviance, 
        ...
        prior.df.for.cutpoints = prior.df.for.cutpoints, niter = niter)
    if (Hess) {
